	...
	<div :class="$style.root">
		...
		<div v-show="gameLoaded" class="_gaps_s">
			...
			<div ref="containerEl" :class="[$style.gameContainer, { [$style.gameOver]: isGameOver && !replaying }]" @contextmenu.stop.prevent @click.stop.prevent="onClick" @touchmove.stop.prevent="onTouchmove" @touchend="onTouchend" @mousemove="onMousemove">
				<img v-if="isDark" src="/client-assets/drop-and-fusion/frame-dark.svg" :class="$style.mainFrameImg"/>
				...
				<div ref="effectsEl" :class="$style.effectsLayer"></div>
				<Transition
				...
				<div v-if="replaying" :class="$style.replayIndicator"><span :class="$style.replayIndicatorText"><Icon name="player-play" /> {{ i18n.ts.replaying }}</span></div>
			</div>
			...
			<div v-if="replaying" class="_woodenFrame">
				...
				<div class="_woodenFrameInner">
					<div class="_buttonsCenter" :class="$style.actionButtons">
						<MkButton :class="$style.actionButton" @click="endReplay"><Icon name="player-stop" /> {{ i18n.ts.endReplay }}</MkButton>
						<MkButton :class="$style.actionButton" :primary="replayPlaybackRate === 4" @click="replayPlaybackRate = replayPlaybackRate === 4 ? 1 : 4"><Icon name="player-track-next" /> x4</MkButton>
						<MkButton :class="$style.actionButton" :primary="replayPlaybackRate === 16" @click="replayPlaybackRate = replayPlaybackRate === 16 ? 1 : 16"><Icon name="player-track-next" /> x16</MkButton>
					</div>
				</div>
			</div>
			...
			<div v-if="isGameOver" class="_woodenFrame">
				<div class="_woodenFrameInner">
					<div class="_buttonsCenter" :class="$style.actionButtons">
						<MkButton :class="$style.actionButton" primary rounded @click="backToTitle">{{ i18n.ts.backToTitle }}</MkButton>
						<MkButton :class="$style.actionButton" primary rounded @click="replay">{{ i18n.ts.showReplay }}</MkButton>
						<MkButton :class="$style.actionButton" primary rounded @click="downloadImage">{{ i18n.ts.share }}</MkButton>
						<MkButton :class="$style.actionButton" rounded @click="exportLog">{{ i18n.ts.copyReplayData }}</MkButton>
					</div>
				</div>
			</div>
			...
			<div style="display: flex;">
				...
				<div class="_woodenFrame" style="margin-left: auto;">
					<div class="_woodenFrameInner" style="text-align: center;">
						<div @click="showConfig = !showConfig"><Icon name="settings" /></div>
					</div>
				</div>
			</div>
			...
			<div v-if="showConfig" class="_woodenFrame">
				<div class="_woodenFrameInner">
					<div class="_gaps">
						<MkRange v-model="bgmVolume" :min="0" :max="1" :step="0.01" :textConverter="(v) => `${Math.floor(v * 100)}%`" @dragEnded="(v) => updateSettings('bgmVolume', v)">
							<template #label>BGM {{ i18n.ts.volume }}</template>
						</MkRange>
						<MkRange v-model="sfxVolume" :min="0" :max="1" :step="0.01" :textConverter="(v) => `${Math.floor(v * 100)}%`" @dragEnded="(v) => updateSettings('sfxVolume', v)">
				...
				<div class="_woodenFrameInner">
					...
					<div>
						<div v-for="(mono, i) in game.monoDefinitions.sort((a, b) => a.level - b.level)" :key="mono.id" style="display: inline-block;">
							...
							<div v-if="i < game.monoDefinitions.length - 1" style="display: inline-block; margin-left: 4px; vertical-align: bottom;"><Icon name="arrow-right" /></div>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="_woodenFrame">
				<div class="_woodenFrameInner">
					<MkButton v-if="!isGameOver && !replaying" :class="$style.actionButton" full danger @click="surrender">{{ i18n.ts.surrender }}</MkButton>
					<MkButton v-else :class="$style.actionButton" full @click="restart">{{ i18n.ts.gameRetry }}</MkButton>
				</div>
			</div>

			<div v-if="showSurrenderConfirm" :class="$style.confirmOverlay" @click.self="cancelSurrender">
				<div :class="$style.confirmDialog">
					<div :class="$style.confirmText">{{ i18n.ts._bubbleGame.confirmSurrender }}</div>
					<div :class="$style.confirmButtons">
						<MkButton :class="$style.actionButton" inline rounded transparent @click="cancelSurrender">{{ i18n.ts._bubbleGame.confirmCancel }}</MkButton>
						<MkButton :class="$style.actionButton" inline rounded primary @click="confirmSurrender">{{ i18n.ts._bubbleGame.confirmOk }}</MkButton>
					</div>
				</div>
			</div>
		</div>
...
import { DropAndFusionGame } from '@/lib/misskey-bubble-game/index.js';
import { useInterval } from '@/utils/useInterval';
import type { Mono } from '@/lib/misskey-bubble-game/index.js';
import MkNumber from '@/components/MkNumber.vue';
import MkButton from '@/components/MkButton.vue';
import MkRange from '@/components/MkRange.vue';
import MkEllipsis from '@/components/MkEllipsis.vue';
...
const effectsEl = ref<HTMLDivElement | null>(null);
const showSurrenderConfirm = ref(false);
const dropperX = ref(0);
const currentPick = ref<{ id: string; mono: Mono } | null>(null);
const stock = ref<{ id: string; mono: Mono }[]>([]);
const holdingStock = ref<{ id: string; mono: Mono } | null>(null);
const score = ref(0);
const combo = ref(0);
...
const bgmVolume = ref(readNumber('bubbleGame.bgmVolume', 0.6));
const sfxVolume = ref(readNumber('bubbleGame.sfxVolume', 0.7));
...
function updateSettings(key: 'bgmVolume' | 'sfxVolume', value: number) {
	if (key === 'bgmVolume') {
		writeNumber('bubbleGame.bgmVolume', value);
		if (bgmNodes?.gainNode) {
			bgmNodes.gainNode.gain.value = props.mute ? 0 : value;
		}
	}
	if (key === 'sfxVolume') {
		writeNumber('bubbleGame.sfxVolume', value);
	}
}
...
function spawnRipple(x: number, y: number) {
	if (!effectsEl.value) return;
const el = document.createElement('div');
el.className = styles.effectRipple;
	el.style.left = `${x - 20}px`;
	el.style.top = `${y - 20}px`;
	effectsEl.value.appendChild(el);
	window.setTimeout(() => el.remove(), 700);
}

function spawnPlusOne(x: number, y: number, text: string) {
	if (!effectsEl.value) return;
const el = document.createElement('div');
el.className = styles.effectPlus;
	el.style.left = `${x}px`;
	el.style.top = `${y}px`;
	el.textContent = `+${text}`;
	effectsEl.value.appendChild(el);
...
async function downloadImage() {
	...
	dcanvas.toBlob(blob => {
		...
		a.href = url;
		a.download = `bubble-game-${Date.now()}.png`;
		a.click();
...
function attachGameEvents() {
	...
	game.addListener('fusioned', (x, y, nextMono, scoreDelta) => {
		...
		spawnRipple(domX, domY);
		spawnPlusOne(domX, domY, scoreDelta + (scoreUnit === 'pt' ? '' : scoreUnit));
	...
	game.addListener('gameOver', () => {
		...
		if (props.gameMode === 'yen') {
			...
			writeNumber('bubbleGame.yenTotal', yenTotal.value);
		}

		if (score.value > (highScore.value ?? 0)) {
			highScore.value = score.value;
			writeNumber(`bubbleGame.highScore.${props.gameMode}`, highScore.value);
		}
	});
}
...
watch(() => props.mute, (value) => {
	if (bgmNodes?.gainNode) {
		bgmNodes.gainNode.gain.value = value ? 0 : bgmVolume.value;
	}
});


onMounted(async () => {
	highScore.value = readNumber(`bubbleGame.highScore.${props.gameMode}`, null);

	if (props.gameMode === 'yen') {
		yenTotal.value = readNumber('bubbleGame.yenTotal', 0);
	}
...
.effectPlus {
	...
	animation: plusOne 0.9s ease-out forwards;
}
